...
string TimerUtil::TimeStamp::ToString() const
{
...
string TimerUtil::TimeStamp::GetWeek() const
{
    return EnWeekName[Week];
}

string TimerUtil::TimeStamp::GetWeek_Ch() const
{
    return ChWeekName[Week];
}

string TimerUtil::TimeStamp::GetMonth() const
{
    return EnMonthName[Month - 1];
}

string TimerUtil::TimeStamp::GetMonth_Ch() const
{
    return ChMonthName[Month - 1];
}

void TimerUtil::PerformanceCounter::BeginCounter(const string& InName)
{
...
void TimerUtil::PerformanceCounter::EndCounter(const string& InName)
{
    if (m_counterResults.find(InName) != m_counterResults.end())
        return;
...
double TimerUtil::PerformanceCounter::GetCounterResult(const string& InName)
{
    if (m_counterResults.find(InName) != m_counterResults.end())
...
TimerUtil::PerformanceScope::PerformanceScope(const string& InScopeName)
{
